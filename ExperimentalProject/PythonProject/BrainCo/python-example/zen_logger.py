...
class ZenLogger(object):
    def __init__(self, logfile):
        super(ZenLogger, self).__init__()
        self.running_stack = []
        self.logfile = None
        self.logfd = None
...
class ZLOG(object):
    ...
    BASEDIR = []
    USE_COLOR = True
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s [%(levelname)s][Zen Python Binding:%(pathname)s:%(lineno)d]>%(message)s", datefmt="%H:%M:%S")
    LOGGER = ZenLogger(None)
    LOGGING = logging.getLogger("Zenlite python binding")
    ...
    def LOG_COLOR(cls, level=logging.INFO, msg=None):
        try:
            if not cls.USE_COLOR:
                raise ValueError
            cls.LOGGING.log(level=level, msg=f'{cls.COLOR[level]}{datetime.datetime.now().strftime("%H:%M:%S")}  {msg}\033[0m')
        except (ValueError, AttributeError) as e:
            cls.LOGGING.log(level=level, msg=f'{datetime.datetime.now().strftime("%H:%M:%S")}{msg}')
...
def set_logdir(dirpath):
    ...
    if not os.path.exists(dirpath):
        os.mkdir(dirpath)
    ST.LOG_DIR = dirpath
    ZLOG.LOGGER.set_logfile(os.path.join(ST.LOG_DIR, ST.LOG_FILE))
...
def log(arg, timestamp=None, desc=""):
    if ZLOG.LOGGER:
        ...
        if isinstance(arg, Exception):
            ...
            ZLOG.LOGGER.log("info", {
                "name": desc or arg.__class__.__name__,
                "traceback": trace_msg,
            }, depth=depth, timestamp=timestamp)
            ZLOG.LOGGING.error(trace_msg)
        elif isinstance(arg, six.string_types):
            ZLOG.LOGGER.log("info", {"name": desc or arg, "traceback": None, "log": arg}, depth=depth, timestamp=timestamp)
            ZLOG.LOGGING.info(arg)
        else:
            ZLOG.LOGGER.log("info", {"name": desc or repr(arg), "traceback": None, "log": repr(arg)}, depth=depth,
                            timestamp=timestamp)
            ZLOG.LOGGING.info(repr(arg))


def logwrap(f):
    return Logwrap(f, ZLOG.LOGGER)
...
def using(path):
    if not os.path.isabs(path):
        ...
        if os.path.exists(abspath):
            path = abspath
    ZLOG.LOGGING.debug("using path: %s", path)
    if path not in sys.path:
        sys.path.append(path)
    ZLOG.BASEDIR.append(path)
